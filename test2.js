...
let expName = 'test2';  // from the Builder filename that created this script
...
psychoJS.start({
  ...
  expInfo: expInfo,
  resources: [
    {'name': 'img_placeholders/1.jpg', 'path': 'img_placeholders/1.jpg'},
    {'name': 'Smiling_Spock_and_Leila_Kalomi.png', 'path': 'Smiling_Spock_and_Leila_Kalomi.png'},
    {'name': 'img_placeholders/25.jpg', 'path': 'img_placeholders/25.jpg'}
  ]
});
...
async function experimentInit() {
  ...
  Scene = new visual.ImageStim({
    ...
    image : 'Smiling_Spock_and_Leila_Kalomi.png', mask : undefined,
    ori : 0.0, pos : [0, 0], size : [900, 900],
  ...
  image = new visual.ImageStim({
    ...
    image : 'img_placeholders/25.jpg', mask : undefined,
    ori : 0.0, pos : [(- 100), 0], size : [100, 100],
    color : new util.Color([1,1,1]), opacity : undefined,
    flipHoriz : false, flipVert : false,
  ...
  image_2 = new visual.ImageStim({
    ...
    image : 'img_placeholders/1.jpg', mask : undefined,
...
function trialRoutineEachFrame() {
  return async function () {
    ...
    // *Scene* updates
    if (t >= 0.5 && Scene.status === PsychoJS.Status.NOT_STARTED) {
    ...
    if (Scene.status === PsychoJS.Status.STARTED && t >= frameRemains) {
...
function ResponseRoutineBegin(snapshot) {
  return async function () {
    ...
    ResponseComponents.push(image);
    ResponseComponents.push(image_2);
...
function ResponseRoutineEachFrame() {
  return async function () {
    ...
    if (mouse.status === PsychoJS.Status.STARTED && Boolean((continueRoutine == False))) {
    ...
    if (mouse.status === PsychoJS.Status.STARTED) {  // only update if started and not finished!
      ...
      if (!_mouseButtons.every( (e,i,) => (e == prevButtonState[i]) )) { // button state changed?
        ...
        if (_mouseButtons.reduce( (e, acc) => (e+acc) ) > 0) { // state changed to a new click
          ...
          gotValidClick = false;
          for (const obj of [image, image_2]) {
    ...
    if (image.status === PsychoJS.Status.STARTED && Boolean((gotValidClick == True))) {
    ...
    if (image_2.status === PsychoJS.Status.STARTED && Boolean((gotValidClick == True))) {
